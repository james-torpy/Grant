...
outPath="$resultsDir/4.star-cuffdiff_ercc_protocol/$samplename.$outType"
...
#perform analysic on bam files using the reference genome and annotation file:
	cufflinks_line="cufflinks -p $numcores -o $outDir --library-type fr-firststrand $inFile"
...
#submit cufflinks job with rame 'CUFFLINKS_$uniqueID' to 10 cluster cores, hold this job until STAR_$uniqueID is complete:
	qsub -N CUFFLINKS_$uniqueID -hold_jid STAR_$uniqueID -wd $logDir -b y -j y -R y -P GenomeInformatics -pe smp $numcores -V $cufflinks_line
